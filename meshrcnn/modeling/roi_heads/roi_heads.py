...
from detectron2.modeling.roi_heads.roi_heads import StandardROIHeads, select_foreground_proposals, select_proposals_with_visible_keypoints
...
class MeshRCNNROIHeads(StandardROIHeads):
    ...
    def _forward_keypoint(self, features, instances):
        ...
        if self.training:
            ...
            keypoint_logits = self.keypoint_head.layers(keypoint_features)
            normalizer = (
                None if self.keypoint_head.loss_normalizer == "visible" else num_images * self.keypoint_head.loss_normalizer
            )
            loss = keypoint_rcnn_loss(keypoint_logits,proposals, normalizer=normalizer)
            return {"loss_keypoint": loss * self.keypoint_head.loss_weight}
        else:
            pred_boxes = [x.pred_boxes for x in instances]
